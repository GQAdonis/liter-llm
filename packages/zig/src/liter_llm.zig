...
// alef:hash:d95639135cc9bbc5bd2060cff9362edbb35855e6ef3cf827ffaab30bc9b44bfd
// alef:handle-abi:1
...
pub const DefaultClient = struct {
    ...
    pub fn wait_for_batch(self: *DefaultClient, batch_id: []const u8, config: []const u8) (CatalogRefreshError||error{OutOfMemory,HandleClosed})![]u8 {
        const handle = self._handle;
        if (handle == 0) return error.HandleClosed;
        const batch_id_z = try std.heap.c_allocator.dupeZ(u8, batch_id);
        defer std.heap.c_allocator.free(batch_id_z);
