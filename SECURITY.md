...
This library talks to third-party LLM providers. In scope: leakage of API keys through logs, error messages, or telemetry; unsafe handling of provider responses (including tool-call payloads treated as trusted); request construction that permits header or URL injection; and TLS or certificate verification that can be weakened by configuration.
