...
// alef:hash:5c301083bae40753f77dac6facf586012fe8229e568abbfadcc47301d582afd5
...
describe("streaming", () => {
	...
	it("edge_stream_function_call: Streaming chat completion with tool/function call chunks", async () => {
		...
		const stream = await client.chatStream({ messages: [{ content: "What's the weather?", role: "user" }], model: "gpt-4", tools: [{ function: { name: "get_weather", parameters: { properties: { city: { type: "string" } }, type: "object" } }, toolType: ToolType.Function }] });
		const chunks: any[] = [];
    for await (const _chunk of stream) { chunks.push(_chunk); }
    expect(chunks.length).toBeGreaterThanOrEqual(2);
