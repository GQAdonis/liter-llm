---
description: "The 20 LiterLlmError variants, HTTP status mapping, transient classification, retry behaviour, and links to per-language exception types."
...
Every liter-llm client and the proxy return the same error taxonomy, defined by the `LiterLlmError` enum in `crates/liter-llm/src/error.rs`. Twenty variants cover authentication, rate limits, payload problems, transport failures, idempotency conflicts, and internal bugs. Language bindings map each variant to an idiomatic exception type but preserve the original semantics.

This page is the canonical reference. See [API Reference](/reference/api-python/) for the per-language exception names.

## Variants

The 20 variants, their typical cause, and whether the Tower middleware treats them as transient:
...
| `OutboundForbidden`     | The active outbound policy (`[security].outbound_policy`) blocked a request URL, e.g. a private-range address under `deny_private`. | no         |
| `IdempotencyConflict`   | The same `Idempotency-Key` was reused with a different request body. Returns 409.                                 | no         |
| `IdempotencyInFlight`   | Another request with the same `Idempotency-Key` is currently in flight. Returns 409. Not auto-retried by the Tower middleware — the caller should wait briefly and retry. | no         |
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Rust       | `LiterLlmError` enum with 20 variants.                                                                                                      | 1:1 with the canonical list. `is_transient()` and `error_type()` available.                                                                                                                                                                                              |
| Python     | Exception hierarchy rooted at `LlmError`.                                                                                                   | 19 classes (every variant except `InternalError`, which surfaces as the base `LlmError`). `ContextWindowExceededError` and `ContentPolicyError` inherit from `BadRequestError`.                                                                                          |
