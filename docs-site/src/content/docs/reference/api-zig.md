...
## Zig API Reference <span class="version-badge">v1.16.0</span>
...
| `accessKeyId` | `\[:0\]const u8?` | `null` | Explicit AWS access key ID. |
| `secretAccessKey` | `\[:0\]const u8?` | `null` | Explicit AWS secret access key. |
| `sessionToken` | `\[:0\]const u8?` | `null` | Explicit AWS session token (temporary credentials). |
...
```zig
pub fn fetchBatchForPolling(self: *const DefaultClient, batch_id: [:0]const u8) LiterLlmError!BatchObject
```
...
| `apiKey` | `\[:0\]const u8?` | `null` | API key for authentication. |
| `baseUrl` | `\[:0\]const u8?` | `null` | Override base URL. When set, all requests go here and provider auto-detection is skipped. |
| `timeoutSecs` | `u64?` | `null` | Request timeout, in seconds. |
| `maxRetries` | `u32?` | `null` | Maximum number of retries on 429 / 5xx responses. |
| `temperature` | `f64?` | `null` | Sampling temperature for requests built from this config. |
| `maxTokens` | `u64?` | `null` | Maximum number of tokens to generate for requests built from this config. |
...
| `cooldownSecs` | `u64?` | `null` | Cooldown duration after transient errors, in seconds. |
| `healthCheckSecs` | `u64?` | `null` | Background health check interval, in seconds. |
...
```zig
pub fn providers(self: *const LlmConfig) []const LlmProviderConfig
```

**Example:**

```zig
const result = instance.providers();
```

**Returns:** `[]const LlmProviderConfig`
