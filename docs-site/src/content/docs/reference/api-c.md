...
## C API Reference <span class="version-badge">v1.15.0</span>
...
| `reasoning_effort` | `LiterllmReasoningEffort*` | `NULL` | Reasoning effort level (minimal, low, medium, high, max) for extended-thinking models. |
...
| `providers` | `LiterllmLlmProviderConfig**` | `NULL` | Custom provider configurations, in addition to the built-in providers. |
| `cache` | `LiterllmLlmCacheConfig*` | `NULL` | Response cache configuration. |
| `budget` | `LiterllmLlmBudgetConfig*` | `NULL` | Budget enforcement configuration. |
| `rate_limit` | `LiterllmLlmRateLimitConfig*` | `NULL` | Per-model rate limiting configuration. |
...
| `bedrock` | `LiterllmBedrockConfig*` | `NULL` | AWS Bedrock configuration (region, credentials, cross-region routing). |
...
```c
LiterllmLlmProviderConfig* literllm_providers();
```

**Example:**

```c
LiterllmLlmProviderConfig* result = literllm_providers(instance);
```

**Returns:** `LiterllmLlmProviderConfig*`
