...
<!-- alef:hash:47c494227fcdedd09e82f096077430d2b80428d2554da85128c9fe6ff6e01d7f -->
<!-- To regenerate: alef docs -->
<!-- To verify freshness: alef verify -->

## C API Reference <span class="version-badge">v1.18.0</span>
...
```c
LITERLLMAlefHandle* literllm_all_providers();
```

**Example:**

```c
LITERLLMAlefHandle* result = literllm_all_providers();
```
...
| `tool_calls` | `LITERLLMAlefHandle*` | `NULL` | Tool calls the model wants to execute, if any. |
...
| `data` | `LITERLLMAlefHandle*` | `NULL` | List of batch objects. |
...
| `choices` | `LITERLLMAlefHandle*` | `NULL` | Streaming choices (delta updates). |
...
| `messages` | `LITERLLMAlefHandle*` | `NULL` | Conversation history from oldest to newest. |
...
| `tools` | `LITERLLMAlefHandle*` | `NULL` | Tools the model can invoke. |
...
| `modalities` | `LITERLLMAlefHandle*` | `NULL` | Output modalities to request from the model. For OpenAI audio models, pass `["text", "audio"]`. Vertex AI / Gemini translates these to `generationConfig.responseModalities` (uppercase). |
...
| `metadata` | `void*` | `NULL` | Developer-defined tags attached to the completion. |
| `prediction` | `void*` | `NULL` | Predicted output, for latency reduction when much of the response is known ahead of time. Untyped: the shape is provider-defined and still evolving, and the value is forwarded verbatim. |
| `audio` | `void*` | `NULL` | Audio output parameters, required when `modalities` includes `audio`. Untyped for the same reason as `prediction`. |
| `web_search_options` | `void*` | `NULL` | Web-search tool configuration for search-enabled models. Untyped for the same reason as `prediction`. |
...
| `choices` | `LITERLLMAlefHandle*` | `NULL` | List of completion choices. |
...
| `logprobs` | `void*` | `NULL` | Per-token log probabilities, when the request asked for them. Required by OpenAI's response schema as an always-present, nullable key, so this is deliberately not `skip_serializing_if`. |
...
| `tools` | `LITERLLMAlefHandle*` | `NULL` | Available tools the model can use. |
...
| `data` | `LITERLLMAlefHandle*` | — | List of embeddings. |
...
|-------|------|---------|-------------|
| `object` | `const char*` | — | Object type (always `"list"`). |
| `data` | `LITERLLMAlefHandle*` | `NULL` | List of file objects. |
| `has_more` | `int32_t*` | `NULL` | Whether more results are available. |
...
| `data` | `LITERLLMAlefHandle*` | `NULL` | List of generated images. |
...
| `headers` | `void*` | `NULL` | Extra headers sent on every request. |
| `providers` | `LITERLLMAlefHandle*` | `NULL` | Custom provider configurations, in addition to the built-in providers. |
...
| `tiers` | `LITERLLMAlefHandle*` | `NULL` | Context-tiered pricing overrides, sorted by ascending `min_context_tokens`. Empty when the model has flat pricing. |
...
| `data` | `LITERLLMAlefHandle*` | `NULL` | List of available models. |
...
| `results` | `LITERLLMAlefHandle*` | — | Results for each input string. |
...
| `images` | `LITERLLMAlefHandle*` | `/* serde(default) */` | Embedded images extracted from the page (if `include_image_base64` was true). |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `pages` | `LITERLLMAlefHandle*` | — | Extracted pages in order. |
...
| `documents` | `LITERLLMAlefHandle*` | `NULL` | Documents to rerank. |
...
| `results` | `LITERLLMAlefHandle*` | — | Reranked documents in order of relevance. |
...
| `output` | `LITERLLMAlefHandle*` | `NULL` | Output items from the response. |
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `results` | `LITERLLMAlefHandle*` | — | List of search results. |
...
| `tool_calls` | `LITERLLMAlefHandle*` | `NULL` | Partial tool calls being streamed. |
...
| `segments` | `LITERLLMAlefHandle*` | `NULL` | Detailed segment-level transcription (if response_format is "verbose_json"). |
...
| `LITERLLM_PARTS` | Array of content parts (text, images, documents, audio). — Fields: `0`: `LITERLLMAlefHandle*` |
...
| `LITERLLM_PARTS` | Structured parts — text, refusals, output images, output audio. — Fields: `0`: `LITERLLMAlefHandle*` |
...
| `LITERLLM_MULTIMODAL` | Text and image parts for a single multimodal embedding. — Fields: `0`: `LITERLLMAlefHandle*` |
