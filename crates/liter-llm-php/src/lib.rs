...
// alef:hash:dae1f93de51dd9d552a60d835722b89afc6d389daed4f2f96c39537718359f29
...
impl CustomProviderConfig {
    ...
    #[php(constructor)]
    pub fn new(name: String, baseUrl: String, authHeader: &AuthHeaderFormat, modelPrefixes: Vec<String>) -> Self {
        Self {
            name,
            base_url: baseUrl,
            auth_header: authHeader.clone(),
    ...
    pub fn get_auth_header(&self) -> AuthHeaderFormat {
...
// `AuthHeaderFormat` is EXTERNALLY tagged in the core crate, so serde writes a unit variant as a
// bare string and a data variant as a single-keyed object whose value IS the payload -- never the
// flat `{"type": ...}` object this class stores its state in. A derived impl therefore
...
impl serde::Serialize for AuthHeaderFormat {
        ...
        S: serde::Serializer,
    {
        match self.type_tag.as_str() {
            "Bearer" => serializer.serialize_str("Bearer"),
            "ApiKey" => {
                use serde::ser::SerializeMap;
                let mut map = serializer.serialize_map(Some(1))?;
                map.serialize_entry("ApiKey", self.api_key.as_deref().unwrap_or_default())?;
                map.end()
            }
...
impl<'de> serde::Deserialize<'de> for AuthHeaderFormat {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        impl<'de> serde::de::Visitor<'de> for AlefExternalVisitor {
            type Value = AuthHeaderFormat;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("a string or an object for AuthHeaderFormat")
            }
            ...
            {
                match value {
                    "Bearer" => Ok(AuthHeaderFormat {
                        type_tag: "Bearer".to_string(),
                        api_key: None,
                    }),
                    "None" => Ok(AuthHeaderFormat {
                        type_tag: "None".to_string(),
                        api_key: None,
                    }),
                    "" => Ok(AuthHeaderFormat::default()),
                    value => Err(serde::de::Error::unknown_variant(value, &["Bearer", "ApiKey", "None"])),
                }
            }
                ...
                A: serde::de::MapAccess<'de>,
            {
                ...
                if entries.iter().any(|(key, _)| key == "type") {
                    let mut value = AuthHeaderFormat::default();
                    for (key, entry) in entries {
                        match key.as_str() {
                            "type" => value.type_tag = entry.as_str().unwrap_or_default().to_string(),
                            "api_key" => value.api_key = entry.as_str().map(str::to_string),
                            _ => {}
                        }
                    }
                    return Ok(value);
                ...
                match entries.into_iter().next() {
                    Some((key, entry)) if key == "ApiKey" => Ok(AuthHeaderFormat {
                        type_tag: "ApiKey".to_string(),
                        api_key: entry.as_str().map(str::to_string),
                    }),
                    Some((key, _)) => Err(serde::de::Error::unknown_variant(
                        key.as_str(),
                        &["Bearer", "ApiKey", "None"],
                    )),
                    None => Ok(AuthHeaderFormat::default()),
                }
            }
        }
...
impl AuthHeaderFormat {
    ...
    pub fn from_json(json: String) -> PhpResult<Self> {
        let value: Self = serde_json::from_str(&json).map_err(|e| PhpException::default(e.to_string()))?;
        if !matches!(value.type_tag.as_str(), "Bearer" | "ApiKey" | "None") {
            return Err(PhpException::default(format!(
                "Invalid AuthHeaderFormat tag: {}",
                value.type_tag,
            )));
        }
        Ok(value)
    ...
    #[php(name = "bearer")]
    pub fn _factory_bearer() -> Self {
        liter_llm::provider::custom::AuthHeaderFormat::Bearer.into()
    }

    #[php(name = "apiKey")]
    pub fn _factory_api_key(apiKey: String) -> Self {
        liter_llm::provider::custom::AuthHeaderFormat::ApiKey(apiKey).into()
    }

    #[php(name = "none")]
    pub fn _factory_none() -> Self {
        liter_llm::provider::custom::AuthHeaderFormat::None.into()
    }
...
impl From<AuthHeaderFormat> for liter_llm::provider::custom::AuthHeaderFormat {
    fn from(val: AuthHeaderFormat) -> Self {
        match val.type_tag.as_str() {
            ...
            _ => liter_llm::provider::custom::AuthHeaderFormat::default(),
        }
    }
}
...
pub extern "C" fn get_module() -> *mut ::ext_php_rs::zend::ModuleEntry {
    ...
    __EXT_PHP_RS_MODULE_ENTRY.get_or_init(|| {
        let builder = ::ext_php_rs::builders::ModuleBuilder::new("liter_llm", "2.1.1")
