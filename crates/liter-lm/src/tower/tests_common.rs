...
impl LiterLmErrorKind {
    pub fn into_error(&self) -> LiterLmError {
        match self {
...
pub struct MockClient {
    ...
    chat_error: Option<Arc<LiterLmErrorKind>>,
...
impl MockClient {
    fn new_with_error(error: Option<LiterLmErrorKind>) -> Self {
        Self {
    ...
    pub fn failing_rate_limited() -> Self {
        Self::new_with_error(Some(LiterLmErrorKind::RateLimited {
            message: "too many requests".into(),
        }))
    }

    pub fn failing_service_unavailable() -> Self {
        Self::new_with_error(Some(LiterLmErrorKind::ServiceUnavailable { message: "503".into() }))
    }

    pub fn failing_auth() -> Self {
        Self::new_with_error(Some(LiterLmErrorKind::Authentication {
            message: "invalid key".into(),
        }))
    }

    pub fn failing_timeout() -> Self {
        Self::new_with_error(Some(LiterLmErrorKind::Timeout))
    }
}
