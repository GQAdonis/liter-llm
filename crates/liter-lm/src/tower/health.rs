...
//! requests are immediately rejected with [`LiterLmError::ServiceUnavailable`].
//!
    ...
    S::Future: Send + 'static,
{
    ...
    fn poll_ready(&mut self, cx: &mut Context<'_>) -> Poll<Result<()>> {
        if !self.healthy.load(Ordering::Acquire) {
            return Poll::Ready(Err(LiterLmError::ServiceUnavailable {
    ...
    fn call(&mut self, req: LlmRequest) -> Self::Future {
        if !self.healthy.load(Ordering::Acquire) {
            return Box::pin(async {
                Err(LiterLmError::ServiceUnavailable {
                    message: "service is unhealthy (health check failed)".into(),
                })
            });
        }
