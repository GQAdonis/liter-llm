...
fn get_or_load_tokenizer(model: &str) -> Result<Arc<Tokenizer>> {
    ...
    // Fast path: read lock
    {
        let cache = TOKENIZER_CACHE.read().map_err(|e| LiterLmError::BadRequest {
    ...
    let mut cache = TOKENIZER_CACHE.write().map_err(|e| LiterLmError::BadRequest {
        message: format!("tokenizer cache lock poisoned: {e}"),
    })?;
    if let Some(tok) = cache.get(tokenizer_id) {
        return Ok(Arc::clone(tok));
    ...
    let tokenizer = Tokenizer::from_pretrained(tokenizer_id, None).map_err(|e| LiterLmError::BadRequest {
...
/// # Errors
///
/// Returns [`LiterLmError::BadRequest`] if the tokenizer cannot be loaded
/// (e.g. network failure on first use) or if tokenization itself fails.
pub fn count_tokens(model: &str, text: &str) -> Result<usize> {
    let tokenizer = get_or_load_tokenizer(model)?;
    let encoding = tokenizer.encode(text, false).map_err(|e| LiterLmError::BadRequest {
...
/// # Errors
///
/// Returns [`LiterLmError::BadRequest`] if the tokenizer cannot be loaded or
