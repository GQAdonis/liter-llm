...
/// `"{provider}/{model}"` key and its bare alias, to the same entry.
///
/// Deliberately does not assert on price literals: those are regenerated by
/// the scheduled models.dev catalog sync and are not what this test exists
/// to prove (see `registry_from_catalog_str`, which clones the identical
/// `ModelPricing` value into both the combined-key and bare-alias registry
/// slots). `model_info` is compared in full (via `ModelInfo`'s `PartialEq`)
/// rather than cherry-picking fields, so any future field added to the
/// pricing/capability projection stays covered by this invariant.
#[test]
...
/// key and the bare alias, to the same entry. See
/// `openai_model_resolves_via_combined_key_and_bare_alias` for why this
/// intentionally avoids pinning price literals.
#[test]
...
/// and the bare alias, to the same entry. See
/// `openai_model_resolves_via_combined_key_and_bare_alias` for why this
/// intentionally avoids pinning price literals.
#[test]
